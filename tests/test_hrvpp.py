...
class TestHRVPP(unittest.TestCase):
    ...
    def test_get_collections(self):
        ...
        self.assertIn(
            "copernicus_r_utm-wgs84_10_m_hrvpp-vi_p_2017-now_v01", collection_ids
        )
    ...
    @unittest.skipIf(
        ...
        or "WEKEO_USERNAME" not in os.environ
        or "WEKEO_PASSWORD" not in os.environ,
    ...
    def test_download_non_interactive(self):
        ...
        catalogue = Catalogue(self.config_hrvpp).authenticate_non_interactive(
            os.getenv("WEKEO_USERNAME"), os.getenv("WEKEO_PASSWORD")
        )
        tileId = "31UGS"
    ...
    def test_download_s3(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for pf in product.data:
                self.assertTrue(
                    os.path.isfile(os.path.join(prod_dir, os.path.basename(pf.href)))
                )
