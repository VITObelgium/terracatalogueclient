...
class TestClient(unittest.TestCase):
    ...
    def test_convert_parameters_publicationDate(self):
        ...
        # left unbounded
        params = Catalogue._convert_parameters(
            {"publicationDate": (None, dt.date(2020, 1, 1))}
        )
        self.assertEqual("2020-01-01]", params["publicationDate"])

        # right unbounded
        params = Catalogue._convert_parameters(
            {"publicationDate": (dt.date(2021, 1, 1), None)}
        )
        ...
        params = Catalogue._convert_parameters(
            {
                "publicationDate": (
                    dt.datetime(2021, 2, 20, 12, 34, 56),
                    dt.date(2021, 3, 1),
                )
            }
        )
    ...
    def test_parse_date(self):
        self.assertEqual(
            dt.datetime(2021, 4, 16, 16, 15, 14),
            terracatalogueclient.client._parse_date("2021-04-16T16:15:14.243Z"),
        )
        self.assertEqual(
            dt.datetime(2020, 2, 20, 18, 12, 38),
            terracatalogueclient.client._parse_date("2020-02-20T18:12:38Z"),
        )
    ...
    def test_get_product_dir(self):
        ...
        self.assertEqual(
            "/tmp/S2A_20150704T101006_32TML_NDVI_10M_V200",
        ...
        self.assertEqual(
            "/tmp/VI_20161001T092022_S2A_T34SDG-010m_V100_FAPAR",
            Catalogue._get_product_dir("/tmp/", product),
        )
    ...
    def test_can_get_all_features(self):
        def get_response(total_results, items_per_page, last_href):
            r = {
                ...
                "properties": {"links": {"last": [{"href": last_href}]}},
            }
            return r
        ...
        response = get_response(
            375433,
            100,
            "https://services.terrascope.be/catalogue/products?"
