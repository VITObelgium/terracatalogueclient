...
class TestIntegration(unittest.TestCase):
    ...
    def test_get_collections_by_platform(self):
        ...
        for c in collections:
            self.assertTrue(
                any(
                    a["platform"]["platformShortName"] == "Sentinel-1"
                    for a in c.properties["acquisitionInformation"]
                )
            )
    ...
    def test_get_collections_by_bbox(self):
        ...
        collections = list(
            catalogue.get_collections(
                bbox={"west": 4.1, "south": 50, "east": 5.5, "north": 51}
            )
        )
        self.assertTrue(len(collections) > 0)
    ...
    def test_get_products_date(self):
        ...
        products = catalogue.get_products(
            "urn:eop:VITO:TERRASCOPE_S2_FAPAR_V2", start=start, end=end, tileId="31UFS"
        )
        for p in products:
            self.assertTrue(
    ...
    def test_get_products_title(self):
        ...
        products = list(
            catalogue.get_products("urn:eop:VITO:TERRASCOPE_S2_FAPAR_V2", title=title)
        )
            ...
            acquisitionParameters = next(
                iter(
                    [
                        ...
                        for i in p.properties["acquisitionInformation"]
    ...
    def test_get_products_cloudCover(self):
        ...
        for p in products:
            self.assertGreaterEqual(
                cloudCoverMax, p.properties["productInformation"]["cloudCover"]
            )
    ...
    def test_get_products_productGroupId(self):
        ...
        for p in products:
            self.assertEqual(
                productGroupId, p.properties["productInformation"]["productGroupId"]
            )
    ...
    def test_get_products_publicationDate(self):
        ...
        for p in products:
            ...
            self.assertTrue(
                dt.datetime.combine(publicationDate[0], dt.datetime.min.time())
                <= published
                <= publicationDate[1]
            )
    ...
    def test_get_products_modificationDate(self):
        ...
        for p in products:
            ...
            self.assertTrue(
                dt.datetime.combine(modificationDate[0], dt.datetime.min.time())
                <= updated
            )
    ...
    def test_get_products_unsupported_parameter(self):
        ...
        self.assertRaises(
            SearchException, list, products
        )  # getting items from the generator and putting them in a list raises the error
    ...
    def test_get_products_limit(self):
        ...
        self.assertEqual(
            ...
            len(
                list(
                    catalogue.get_products(
                        "urn:eop:VITO:TERRASCOPE_S2_FAPAR_V2", limit=10
                    )
                )
            ),
        )
        self.assertEqual(
            ...
            len(
                list(
                    catalogue.get_products(
                        "urn:eop:VITO:TERRASCOPE_S2_FAPAR_V2", limit=220
                    )
                )
            ),
        )
        ...
        self.assertRaises(
            ...
            list,
            catalogue.get_products(
                "urn:eop:VITO:TERRASCOPE_S2_FAPAR_V2", limit=10_000_000
            ),
        )
    ...
    def test_download_unauthenticated(self):
        catalogue = Catalogue()
        products = list(
            catalogue.get_products("urn:eop:VITO:COP_DEM_GLO_30M_COG", limit=1)
        )
    ...
    @unittest.skipIf(
        int(os.getenv("MANUAL_TESTS", 0)) == 0,
        "Run manually to test download with authentication.",
    )
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for product in products:
                ...
                for pf in product.data:
                    self.assertTrue(
                        os.path.isfile(
                            os.path.join(prod_dir, os.path.basename(pf.href))
                        )
                    )

    @unittest.skipIf(
        int(os.getenv("MANUAL_TESTS", 0)) == 0,
        "Run manually to test download authorization.",
    )
    def test_authorization_download(self):
        ...
        product = products[0]
        self.assertFalse(
            catalogue._is_authorized_to_download_http(product.data[0])
        )  # unauthenticated download not possible
        self.assertTrue(
            catalogue_auth._is_authorized_to_download_http(product.data[0])
        )  # authenticated download possible
